...
class OpenBCIBoard(object):
    ...
    def __init__(self, port=None, baud=115200, filter_data=True,
                 scaled_output=True, daisy=False, aux=False, impedance=False, log=False, timeout=None):
        self.log = log # print_incoming_text needs log
        self.streaming = False
        self.baudrate = baud
        self.timeout = timeout
        if not port:
            while port is None or port == "":
                port = self.find_port()
        self.port = port
    ...
    def _read_serial_binary(self, max_bytes_to_skip=3000):
        ...
        while rep <= max_bytes_to_skip:
            ...
            if self.read_state == 0:
                ...
                while struct.unpack('B', b)[0] != START_BYTE:
                    b = read(1)
